...
draw_board(){
    ...
    if (( ${#LEGAL_MOVEMENTS[@]} != 0 )); then
        echo "Неверный ход!"
        echo "Невозможно костяшку $INPUT_STRING передвинуть на пустую ячейку."
        var numbers_str=""
        for NUMBER in "${LEGAL_MOVEMENTS[@]}"
        do
          numbers_str+="$NUMBER, "
        done
        echo "Можно выбрать: ${numbers_str:0:${#numbers_str}-1}"
    fi
    LEGAL_MOVEMENTS=()
}
...
quit_game(){
    while :
    do
        read -r -n 1 -s -p "Вы действительно хотите выйти [y/n]?"
        case $REPLY in
            y|Y) exit
            ;;
            n|N) return
